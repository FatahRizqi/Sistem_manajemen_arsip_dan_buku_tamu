...
router.post("/surat-keluar/:id_surat_keluar/finalisasi", async (req, res) => {
  const permissionError = await permissionGuard(req, res, pendingMenuPaths, "canCreate");
  ...
  try {
    ...
    const baseBuffer = await generatePdfFromSurat(surat);
    const fileName = `${String(surat.nomor_surat || `surat-keluar-${surat.id_surat_keluar}`).replace(/[\\/:*?"<>|]+/g, "-")}-final.pdf`;
    const pathFile = await uploadPdfBuffer(baseBuffer, fileName, {
      idCabang: surat.id_cabang || req?.auth?.id_cabang || null,
      moduleName: "tte/final"
    });
    await DB.transaction(async trx => {
      ...
      if (existingFlow) {
        await trx("trx_alur_tanda_tangan").where("id_alur_tanda_tangan", existingFlow.id_alur_tanda_tangan).update({
          status_alur: "aktif",
          urutan_aktif: 1,
      ...
      await trx("trx_tracking_surat_keluar").insert({
        id_surat_keluar: oPayload.id_surat_keluar,
        status: surat.status,
        aktivitas: "tte_difinalisasi",
        catatan: oPayload.catatan || "Dokumen final untuk tanda tangan elektronik dibuat",
        tanggal: new Date(),
    ...
    await Logging(error, {
      ...
      response: "Dokumen gagal difinalisasi",
      user: req?.auth?.nama_pengguna || ""
    });
    return res.status(500).json({
      status: status.BAD_REQUEST,
      message: "Dokumen gagal difinalisasi"
    });
  }
});
